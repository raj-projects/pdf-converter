...
export default function ExtractImagesPage() {
  const handleProcess = async (files: File[], options: any) => {
    ...
    const response = await fetch("/api/tools/extract-images", {
      method: "POST",
      body: formData,
    })

    if (!response.ok) throw new Error("Extract images failed")
    ...
    <div className="min-h-screen py-12 md:py-20 bg-gradient-to-b from-background via-background to-background">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <ToolTemplate
          ...
          description="Extract all embedded images from your PDF file. Returns a ZIP with all images."
